...
def upgrade():
    ...
    with op.batch_alter_table("celery_users", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_celery_users_created_at"), ["created_at"], unique=False
        )
    ...
    with op.batch_alter_table("celery_workflows", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_celery_workflows_created_at"), ["created_at"], unique=False
        )
        batch_op.create_index(
            batch_op.f("ix_celery_workflows_tripo_task_id"),
            ["tripo_task_id"],
            unique=False,
        )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["workflow_id"],
            ["celery_workflows.id"],
            name=op.f("fk_celery_tasks_workflow_id_celery_workflows"),
            ondelete="cascade",
        ),
    ...
    with op.batch_alter_table("celery_tasks", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_celery_tasks_created_at"), ["created_at"], unique=False
        )
        batch_op.create_index(
            batch_op.f("ix_celery_tasks_workflow_id"), ["workflow_id"], unique=False
        )
